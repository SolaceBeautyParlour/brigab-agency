...
/**
 * Reads a video file's actual duration client-side, before it's ever
 * uploaded — this is what lets us reject a 3-minute clip instantly instead
 * of wasting the manager's data uploading it first.
 */
...
export default function RoomMedia({ room, managerMode, onChanged }) {
  const [items, setItems] = useState(room.media || []);
  ...
  async function handleFile(e) {
    ...
    setBusy(true);
    try {
      const created = await api.uploadRoomMedia(room.id, file);
        ...
        {items.map((m) => (
          <div key={m.id} className="relative shrink-0 w-16 h-16 rounded-md overflow-hidden bg-ink/5 border border-ink/10">
            {m.resource_type === "video" ? (
              ...
              <img src={m.url} alt={`${room.room_code} photo`} className="w-full h-full object-cover" />
            )}
